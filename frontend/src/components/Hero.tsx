  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.hero-badge', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' })
        .fromTo('.hero-headline', { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.3')
        .fromTo('.hero-sub', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '-=0.4')
        .fromTo('.hero-ctas > *', { opacity: 0, y: 15 }, { opacity: 1, y: 0, stagger: 0.1, duration: 0.5, ease: 'power2.out' }, '-=0.3')
        .fromTo('.hero-social > *', { opacity: 0, y: 10 }, { opacity: 1, y: 0, stagger: 0.08, duration: 0.4, ease: 'power2.out' }, '-=0.2')
        .fromTo('.hero-trust > *', { opacity: 0 }, { opacity: 1, stagger: 0.06, duration: 0.3, ease: 'power2.out' }, '-=0.1');

      // Scroll-triggered batch reveals
      ScrollTrigger.batch('.reveal-card', {
        onEnter: (elements) => {
          gsap.fromTo(elements, { opacity: 0, y: 24 }, { opacity: 1, y: 0, stagger: 0.08, duration: 0.6, ease: 'power2.out', overwrite: true });
        },
        start: 'top 88%',
        once: true,
      });
      ...
      ScrollTrigger.batch('.reveal-step', {
        onEnter: (elements) => {
          gsap.fromTo(elements, { opacity: 0, y: 20 }, { opacity: 1, y: 0, stagger: 0.12, duration: 0.5, ease: 'power2.out', overwrite: true });
        },
        start: 'top 85%',
        once: true,
      });
      ...
      gsap.fromTo('.stats-section',
        ...
        { scrollTrigger: { trigger: statsRef.current, start: 'top 80%' }, opacity: 1, duration: 1, ease: 'power2.out' }
      );
    }, heroRef);
    ...
    <div ref={heroRef} className="min-h-screen overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 flex flex-col items-center">
          ...
          {/* Globe */}
          <div className="w-full max-w-xl mx-auto mb-6 reveal-card">
            <GlobeMap />
          </div>

          {/* Badge */}
          <div className="hero-badge opacity-0 inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[var(--primary)]/30 bg-[var(--primary)]/5 mb-8">
          ...
          {/* Headline */}
          <h1 className="hero-headline opacity-0 text-center text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-black tracking-tight leading-[1.05] mb-6">
          ...
          {/* Sub */}
          <p className="hero-sub opacity-0 text-center text-lg sm:text-xl text-[var(--muted)] max-w-2xl mb-10 leading-relaxed">
            ISP, Residential, Mobile &amp; Datacenter proxies — delivered in seconds.
            <br className="hidden sm:block" />Leave no footprint.
          </p>

          {/* CTAs */}
          <div className="hero-ctas flex flex-col sm:flex-row items-center gap-3 mb-6 w-full sm:w-auto opacity-0">
            <Link href="/products"
          ...
          {/* WhatsApp + Telegram */}
          <div className="hero-social flex flex-col sm:flex-row items-center gap-3 mb-10 opacity-0">
            <a href="https://wa.me/2347032981049" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2.5 px-6 py-3 rounded-xl bg-[#25D366] hover:bg-[#1da851] text-white font-bold text-sm transition-all duration-200 min-w-[200px] justify-center">
            ...
            <a href="https://t.me/StyxproxyBot" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2.5 px-6 py-3 rounded-xl bg-[#0088cc] hover:bg-[#006699] text-white font-bold text-sm transition-all duration-200 min-w-[200px] justify-center">
          ...
          {/* Trust indicators */}
          <div className="hero-trust flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-[var(--muted)] text-xs font-medium tracking-wide opacity-0">
            {[
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
          <span className="text-[10px] tracking-[0.3em] uppercase text-[var(--muted)] opacity-40">Scroll</span>
          <div className="w-px h-8 bg-gradient-to-b from-[var(--primary)]/50 to-transparent animate-pulse" />
        </div>
      </section>

      {/* ── STATS STRIP ── */}
      <section ref={statsRef} className="border-y border-[var(--border)] bg-[var(--surface)]">
      ...
      {/* ── FEATURES ── */}
      <section ref={featuresRef} className="py-24 lg:py-32 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((f, i) => (
              <div key={i}
                className="reveal-card p-6 rounded-2xl bg-[var(--card)] border border-[var(--border)] hover:border-[var(--primary)]/40 transition-all duration-200"
              >
      ...
      {/* ── HOW IT WORKS ── */}
      <section ref={howRef} className="py-24 lg:py-32 px-6 bg-[var(--surface)]">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="reveal-step relative p-8 rounded-2xl bg-[var(--card)] border border-[var(--border)] hover:border-[var(--primary)]/40 transition-all duration-200">
      ...
      {/* ── PRODUCTS ── */}
      <section ref={productsRef} className="py-24 lg:py-32 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16">
      ...
      {/* ── FAQ ── */}
      <section ref={faqRef} className="py-24 lg:py-32 px-6">
