...
export default function AdminCharonPage() {
  ...
  const loadFileContent = async (name: string) => {
    const result = await api.getLearnedFileContent(name);
    if (result.error) setError(result.error);
    else setSelectedFile(result.data || null);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-[var(--muted)]">Manage Charon learned knowledge base</p>
      </div>

      {error && (
        <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={() => setError('')} className="text-red-300 hover:text-white ml-4">[x]</button>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-6">
        {(['files', 'write'] as const).map((tab) => (
          ...
          >
            {tab === 'files' ? 'Learned Files' : 'Write New'}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="p-6 rounded-2xl bg-[var(--card)] border border-[var(--border)]">
            ...
            {loading ? (
              ...
              <p className="text-center py-8 text-[var(--muted)]">No learned files yet. Write some content!</p>
            ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-[var(--muted)]">
                          {fmtSize(file.size)} - {fmtDate(file.modified_at)}
                        </p>
                      </div>
                      <button
          ...
          <div className="p-6 rounded-2xl bg-[var(--card)] border border-[var(--border)]">
            ...
            {selectedFile ? (
              ...
              <p className="text-center py-8 text-[var(--muted)]">Select a file to preview its content</p>
            )}
          </div>
        </div>
      ) : (
