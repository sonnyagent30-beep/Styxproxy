...
async def get_funnel_analytics(
    from_date: Optional[datetime] = Query(None, description="Start date for analytics (defaults to 30 days ago)"),
    to_date: Optional[datetime] = Query(None, description="End date for analytics (defaults to now)"),
    ...
    for event_name in FUNNEL_STAGES:
        ...
        result = await session.execute(stmt, {"event_name": event_name, "from_date": from_date, "to_date": to_date})
        ...
        stages.append(FunnelStage(event_name=event_name, count=count, conversion_rate=conversion_rate))

        previous_count = count
        total_events += count

    return AnalyticsFunnelResponse(stages=stages, total_events=total_events, period_start=from_date, period_end=to_date)
...
async def list_analytics_events(
    ...
    return AnalyticsEventsListResponse(events=events, total=total, page=(offset // limit) + 1, page_size=limit)
