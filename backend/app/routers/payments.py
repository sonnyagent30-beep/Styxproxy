...
async def initiate_payment(request: PaymentInitiateRequest, session: AsyncSession = Depends(get_session), current_user: dict = Depends(get_current_account)):
    ...
    try:
        result = await create_flutterwave_invoice(amount=total_amount, customer_email=f"{customer.phone}@bunche.ng",
            customer_phone=customer.phone, currency="NGN", callback_url=request.callback_url, description=f"Payment for {request.plan_code}")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to initiate payment: {str(e)}")
    return PaymentInitiateResponse(payment_id=str(uuid.uuid4()), checkout_url=result.get("checkout_url", ""),
        amount_ngn=total_amount, expires_at=datetime.utcnow() + timedelta(minutes=30))
...
async def get_payment_status(tx_ref: str, session: AsyncSession = Depends(get_session), current_user: dict = Depends(get_current_account)):
    ...
    return PaymentStatusResponse(tx_ref=tx_ref, status=payment_data.get("status", "unknown"),
        amount=payment_data.get("amount", 0), currency=payment_data.get("currency", "NGN"))
