...
async def register_platform(
    ...
    """Register a new platform account for the authenticated user."""
...
async def get_my_account(
    ...
    if not customer:
        return PlatformMeResponse(
            customer=None,
            accounts=[PlatformAccountResponse.model_validate(platform_account)],
        )
...
async def merge_accounts(
    ...
    """Request to merge two platform accounts."""
    customer = current_user["customer"]
    ...
    stmt = select(PlatformAccount).where(
        PlatformAccount.id == request.source_account_id,
        PlatformAccount.customer_id == customer.id,
    )
    result = await session.execute(stmt)
    ...
    stmt = select(PlatformAccount).where(
        PlatformAccount.id == request.target_account_id,
        PlatformAccount.customer_id == customer.id,
    )
    result = await session.execute(stmt)
