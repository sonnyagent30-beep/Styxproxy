...
async def claim_trial(request: TrialClaimRequest, session: AsyncSession = Depends(get_session), current_user: dict = Depends(get_current_account)):
    ...
    if not await check_trial_limit(session, customer.phone):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Daily trial limit reached")
    try:
    ...
    stmt = select(BuncheCredential).where(BuncheCredential.id == trial.bunche_credential_id)
    ...
    return TrialClaimResponse(trial_id=trial.id, status=trial.status or "active", bunche_credential=TrialCredentialResponse(bun_username=credential.bun_username, upstream_proxy_ip=credential.upstream_proxy_ip or "0.0.0.0", upstream_proxy_port=credential.upstream_proxy_port, expires_at=credential.expires_at))
