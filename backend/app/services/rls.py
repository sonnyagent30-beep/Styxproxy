...
def get_migrate_engine():
    ...
    url = f"postgresql+asyncpg://styxproxy_migrate:{_read_migrate_password()}" "@127.0.0.1:5432/styxproxy"
...
def get_superuser_engine():
    ...
    """
    url = f"postgresql+asyncpg://postgres:{_read_superuser_password()}" "@127.0.0.1:5432/styxproxy"
    return create_async_engine(url, echo=False)
...
async def _ensure_styxproxy_app_role(_unused) -> bool:
    ...
    try:
        async with eng_super.begin() as mconn:
            ...
            if not existed:
                await mconn.execute(text("CREATE ROLE styxproxy_app NOLOGIN NOSUPERUSER NOINHERIT"))
                log.info("rls: created role styxproxy_app")
            await mconn.execute(
                text("GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES " "IN SCHEMA public TO styxproxy_app")
            )
            await mconn.execute(text("GRANT USAGE, SELECT ON ALL SEQUENCES " "IN SCHEMA public TO styxproxy_app"))
            await mconn.execute(
                text(
            ...
            await mconn.execute(
                text("ALTER DEFAULT PRIVILEGES IN SCHEMA public " "GRANT USAGE, SELECT ON SEQUENCES TO styxproxy_app")
            )
    finally:
...
async def list_policies(session: AsyncSession) -> tuple[list[RlsPolicy], int, int]:
    ...
    result = await session.execute(__import__("sqlalchemy").select(RlsPolicy).order_by(RlsPolicy.table_name))
...
async def toggle_policy(
    ...
    *,
    ...
    row = (
        await session.execute(__import__("sqlalchemy").select(RlsPolicy).where(RlsPolicy.table_name == table_name))
    ).scalar_one_or_none()
    ...
    try:
        if enable:
            ...
            async with eng.begin() as mconn:
                await mconn.execute(text("ALTER TABLE " + table_name + " ENABLE ROW LEVEL SECURITY"))
                await mconn.execute(text("DROP POLICY IF EXISTS " + policy_name + " ON " + table_name))
                await mconn.execute(text("DROP POLICY IF EXISTS " + admin_policy_name + " ON " + table_name))
            ...
            async with eng.begin() as mconn:
                await mconn.execute(text("DROP POLICY IF EXISTS " + policy_name + " ON " + table_name))
                await mconn.execute(text("DROP POLICY IF EXISTS " + admin_policy_name + " ON " + table_name))
                await mconn.execute(text("ALTER TABLE " + table_name + " DISABLE ROW LEVEL SECURITY"))
    ...
    return {
        ...
        "policies": ([policy_name, admin_policy_name] if enable else []),
...
async def get_bypass_role_status(session: AsyncSession) -> dict:
    ...
    bypass_role_exists_row = (
        await session.execute(text("SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'styxproxy_app')"))
    ).scalar()
    current_user_role_row = (
        await session.execute(
    ...
    return {
        ...
        "current_user_role": (current_user_role_row[1] if current_user_role_row else "unknown"),
        "bypass_role_attr_present": (bool(current_user_role_row[3]) if current_user_role_row else False),
    }
...
async def get_rollout_plan(session: AsyncSession) -> tuple[list[dict], Optional[str], bool]:
    ...
    result = await session.execute(
        ...
        .select(RlsPolicy.table_name, RlsPolicy.policy_enabled, RlsPolicy.applied_at)
        .where(RlsPolicy.table_name.in_([p[1] for p in ROLLOUT_PHASES]))
    )
